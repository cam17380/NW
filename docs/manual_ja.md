...
1. 左側のパレットからデバイス（Router、Switch、Firewall、PC）をドラッグ
2. キャンバス上の任意の場所にドロップ
3. 自動的に名前が付与されます（R1, SW1, FW1, PC1等）
...
ツールバーの「Save」ボタンをクリックすると、現在の全設定がブラウザのlocalStorageに保存されます。

### 12.2 手動読み込み

ツールバーの「Load」ボタンをクリックすると、最後に保存した設定を読み込みます。
